...
from vendor_management_app.services.commands import create_vendor, update_vendor, delete_vendor, create_purchase_order, update_purchase_order, delete_purchase_order , create_historical_performance, update_historical_performance, delete_historical_performance
from vendor_management_app.services.queries import get_vendors, get_purchase_orders, get_historical_performances
...
class VendorApi(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if no_pagination:
            return Response(self.OutputSerializer(vendors, many=True, context={'request': request}).data,
...
class PurchaseOrderApi(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if po_id:
            # If 'po_id' is provided, retrieve a single purchase order
            purchase_order = get_object_or_404(PurchaseOrder, id=po_id)
            serializer = self.OutputSerializer(purchase_order)
            return Response(serializer.data, status=status.HTTP_200_OK)
        elif 'po_id' in self.kwargs:
            # If 'po_id' is provided, retrieve a single purchase order
            purchase_order = get_object_or_404(PurchaseOrder, id=self.kwargs['po_id'])
            serializer = self.OutputSerializer(purchase_order)
            return Response(serializer.data, status=status.HTTP_200_OK)
        elif vendor_id:
            ...
            # If neither 'po_id' nor 'vendor_id' is provided, return all purchase orders
            params = request.GET.dict()
            purchase_orders = get_purchase_orders()
            no_pagination = request.GET.get('no_pagination', False) == 'true'
            if no_pagination:
                return Response(self.OutputSerializer(purchase_orders, many=True, context={'request': request}).data,
                                status=status.HTTP_200_OK)
...
class HistoricalPerformanceApi(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if vendor_id:
            ...
            historical_performances = HistoricalPerformance.objects.filter(vendor=vendor_id)
    ...
    def put(self, request, historical_performance_id, *args, **kwargs):
        ...
        serializer = self.InputSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        historical_performance = update_historical_performance(historical_performance=historical_performance, **serializer.validated_data)
